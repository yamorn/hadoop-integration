package com.sun.hadoopdemo.util;
...
//        ImageUtils.gray("e:/abc.jpg", "e:/abc_gray.jpg");//测试OK
        ...
        // 5-给图片添加文字水印：
        // 方法一：
//        ImageUtils.pressText("我是水印文字", "D:/18.jpg", "D:/188.jpg", "宋体", Font.BOLD, Color.white, 80, 0, 0, 0.5f);//测试OK
//        // 方法二：
...
//        ImageUtils.pressImage("e:/abc2.jpg", "e:/abc.jpg", "e:/abc_pressImage.jpg", 0, 0, 0.5f);//测试OK
        ...
        File file = new File("D:/18.jpg");
        FileInputStream fileInputStream=new FileInputStream(file);
        IOUtils.copyBytes(fileInputStream, out, 1024);
//        ImageUtils.pressText("水印 水印",out.toByteArray(),"D:/test/ii.jpg","宋体", Font.BOLD, Color.white, 80, 0, 0, 0.5f);
        ImageUtils.gray(out.toByteArray(), "D:/test/ii.jpg");//测试OK

//        ImageUtils.pressText("我是水印文字", out.toByteArray(), "D:/188.jpg", "宋体", Font.BOLD, Color.white, 80, 0, 0, 0.5f);//测试OK
    ...
    public static void gray(byte[] srcImage, String destImageFile) {
        ByteArrayInputStream byteArrayInputStream=null;
        try {
            byteArrayInputStream = new ByteArrayInputStream(srcImage);
    ...
    public static void pressText(String pressText,
                                 byte[] srcImage, String destImageFile, String fontName,
                                 int fontStyle, Color color, int fontSize, int x,
                                 int y, float alpha) {
        ByteArrayInputStream byteArrayInputStream=null;
        try {
            byteArrayInputStream = new ByteArrayInputStream(srcImage);
            Image src = ImageIO.read(byteArrayInputStream);
            int width = src.getWidth(null);
            int height = src.getHeight(null);
            BufferedImage image = new BufferedImage(width, height,
                    BufferedImage.TYPE_INT_RGB);
            ...
            g.drawString(pressText, (width - (getLength(pressText) * fontSize))
                    / 2 + x, (height - fontSize) / 2 + y);
            g.dispose();
            File dst = new File(destImageFile);
            System.out.println("=====" + dst.getAbsolutePath());
            ImageIO.write((BufferedImage) image, "JPEG", dst);// 输出到文件流
        } catch (Exception e) {
            e.printStackTrace();
        }finally {
            try {
